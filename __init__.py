    ...
    with HDF5_File(hdf5_filename) as hdf5_file:
        ...
        for module_function, kwargs in analyses:
            if module_function.find(".") != -1:
                ...
                check_functions[module_function]    = getattr(sys.modules["MD_toolkit.primary." + module], "_check_" + function)
            else:
                function                            = module_function
                check_functions[module_function]    = getattr(sys.modules["MD_toolkit.primary"],           "_check_" + function)
...
def analyze_primary_x(hdf5_filename, path, analyses, segment_lister = "MD_toolkit.standard_functions.segments_standard",
    ...
    for module in set([m.split(".")[0] for m in [a[0] for a in analyses if a[0].find(".") != -1]]):
        import_module("MD_toolkit.primary_x." + module)
    ...
    with HDF5_File(hdf5_filename) as hdf5_file:
        ...
        for module_function, kwargs in analyses:
            if module_function.find(".") != -1:
                module, function    = module_function.split(".")
                check_function      = getattr(sys.modules["MD_toolkit.primary_x." + module], "_check_" + function)
            else:
                function            = module_function
                check_function      = getattr(sys.modules["MD_toolkit.primary_x"],           "_check_" + function)
