#!/usr/bin/python
desc = """cython_functions.pyx
    Cython-accelerated functions for various analyses
...
def _cy_distance_pbc(np.ndarray[np.float32_t, ndim = 2] group1_crd,
                     np.ndarray[np.float32_t, ndim = 2] group2_crd,
                     double                             length):
    """ Calculates distance between coordinate sets <group1_crd> and <group2_crd> within a cubic pbc box of <length> """
    cdef np.ndarray[np.float32_t, ndim = 2] distance = np.zeros((group1_crd.shape[0], group2_crd.shape[0]), np.float32)
    cdef np.ndarray[np.float32_t, ndim = 1] d_xyz    = np.zeros(3,                                          np.float32)
...
@cython.boundscheck(False)
@cython.wraparound(False)
def _cy_contact(np.ndarray[np.float32_t, ndim = 2] crd,
                np.ndarray[np.long_t,    ndim = 2] atomcounts,
                np.ndarray[np.long_t,    ndim = 2] indexes):
    ...
    cdef np.ndarray[np.float32_t, ndim = 1] d_xyz       = np.zeros(3,                           np.float32)
    cdef np.ndarray[np.int8_t,    ndim = 1] contact     = np.zeros((n_res * n_res - n_res) / 2, np.int8)
    with nogil:
...
@cython.boundscheck(False)
@cython.wraparound(False)
def _cy_acf(np.ndarray[np.float32_t, ndim = 1] vector,
            np.ndarray[np.float32_t, ndim = 3] rotmat,
    ...
    cdef np.ndarray[np.float32_t, ndim = 2] rotated     = np.zeros((n_frames, 3),    np.float32)
    cdef np.ndarray[np.float32_t, ndim = 1] acf         = np.zeros(index_finite + 1, np.float32)
    with nogil:
