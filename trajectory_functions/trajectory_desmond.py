...
desc = """analyze_desmond.py
    Functions for analysis of Desmond simulations
...
def energy(arguments):
    ...
    start_time      = head[7]
    return [("/" + jobstep + "/time",               data[1:,0] / 1000.),
            ("/" + jobstep + "/energy_total",       data[1:,1]),
            ("/" + jobstep + "/energy_potential",   data[1:,2]),
            ("/" + jobstep + "/energy_kinetic",     data[1:,3]),
            ("/" + jobstep + "/temperature",        data[1:,9]),
            ("/" + jobstep,                         {'date': date,  'time': start_time}),
